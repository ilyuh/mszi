...
int main()
{
	...
	while ( true )
	{
		...
		if ( op == 'e' )
		{
			...
			cout << "Enter the key (10 symbols): ";
			wstring key = strInput();
			wstring msgEnc = enc.Encrypt( msg, key );
			wcout << "Your encrypted message is: " << endl << msgEnc << endl;
			break;
		}
		else if ( op == 'd' )
		{
			...
			cout << "Enter the key (10 symbols): ";
			wstring key = strInput();
			wstring msgUnc = enc.Uncrypt( msg, key );
			wcout << "Your decrypted message is: " << endl << msgUnc << endl;
			break;
		}
		else
			inputIncorrect( "There is no such an option, please retry" );
